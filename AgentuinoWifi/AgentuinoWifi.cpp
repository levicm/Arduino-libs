...
SNMP_API_STAT_CODES AgentuinoWifiClass::begin(char *getCommName, char *setCommName, uint16_t port)
{
	// set community name set/get sizes
...
void AgentuinoWifiClass::listen(void)
{
...
SNMP_API_STAT_CODES AgentuinoWifiClass::requestPdu(SNMP_PDU *pdu)
{
	char *community;
	// sequence length
	byte seqLen;
...
SNMP_API_STAT_CODES AgentuinoWifiClass::responsePdu(SNMP_PDU *pdu)
{
	int32_u u;
	byte i;
	//
...
void AgentuinoWifiClass::onPduReceive(onPduReceiveCallback pduReceived)
{
	_callback = pduReceived;
}

void AgentuinoWifiClass::freePdu(SNMP_PDU *pdu)
{
	//
	memset(pdu->OID.data, 0, SNMP_MAX_OID_LEN);
	memset(pdu->VALUE.data, 0, SNMP_MAX_VALUE_LEN);
